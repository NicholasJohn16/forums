...
class ComForumsTemplateHelper extends LibBaseTemplateHelperAbstract {
    ...
    public function parentForum($category) {
        ...
        if (count($forums) === 0) {
            return translate('COM-FORUMS-NO-FORUMS');
        }

        $options[] = translate('COM-FORUMS-FORUM-SELECT-A-FORUM');
    ...
    public function parentCategory($forum) {
        ...
        if (count($categories) == 0) {
            return translate('COM-FORUMS-NO-CATEGORIES');
        }

        $options[] = translate('COM-FORUMS-CATEGORY-SELECT-A-CATEGORY');
    ...
    public function threads($thread, $createNew = true)
    {
        ...
        if($createNew) {
            $options[] = translate('COM-FORUMS-POST-CREATE-NEW-THREAD');
            $parameters['selected'] = $thread->id;
        } else {
            $options[] = translate('COM-FORUMS-THREAD-SELECT-THREAD');
        }

        $options[-1] = translate('COM-FORUMS-POST-SPECIFY-THREAD');
