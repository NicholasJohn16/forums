...
class ComForumsDomainBehaviorRepliable extends AnDomainBehaviorAbstract
{
	...
	public function getPostCount($entity, $num) {
    ...
    public function resetLastReply($entity) {
    	...
    	if($entity->getIdentifier()->name === 'thread') {
    		...
    		$lastReply = $this->getService('repos://site/forums.post')->getQuery()->where('parent_id', 'IN', $threadIds)->order('creationTime', 'DESC')->fetch();
    	}
